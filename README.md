...
- [ ] 고객에게 값 입력받기 
  - [ ] 구매할 상품과 수량 입력받기 
    - ❗[상품명-수량], 형식에 맞지 않게 입력한 경우 예외 처리하고 다시 입력받기 
    - ❗재고에 없는 상품명을 입력한 경우 예외 처리하고 다시 입력받기 
    - ❗수량이 재고수량을 초과하여 입력한 경우 예외 처리하고 다시 입력받기 
  - [ ] 프로모션 상품에 대해 적게 가져온 경우, 추가 여부 입력받기
    - ❗ Y,N 이외의 값을 입력한 경우 예외 처리하고 다시 입력받기 
    - ➕ 만약 프로모션 상품 재고가 부족하여 수량을 적게 가져온 경우,  정가 결제 여부에 대한 안내로 넘어가기 
  - [ ] 프로모션 상품 재고가 부족하여 혜택없이 결제할 경우, 정가 결제에 대한 여부 입력받기 
    - ❗ Y,N 이외의 값을 입력한 경우 예외 처리하고 다시 입력받기
  - [ ] 멤버십 할인 적용 여부 입력받기 
    - ❗ Y,N 이외의 값을 입력한 경우 예외 처리하고 다시 입력받기
  - [ ] 추가 구매 여부 입력받기 
    - ❗ Y,N 이외의 값을 입력한 경우 예외 처리하고 다시 입력받기
  - [ ] 공통 예외 처리
    - ❗ 사용자가 값을 입력하지 않고 빈 문자열을 입력한 경우 예외 처리하고 다시 입력받기 

### 프로모션 관리하기 
- [ ] 프로모션 파일을 읽고 프로모션 정보 저장하기 
  - 프로모션 이름, 구매조건, 증정량, 시작날짜 그리고 종료 날짜 설정
- [ ] 프로모션 상품 구매량과 재고량으로 프로모션 상품의 구매 가능 갯수 계산하기 
  - [ ] 적게 들거온 경우 -(수량) 형태로 반환하기
  - [ ] 프로모션 재고보다 많이 들고온 경우 +(수량) 형태로 반환하기 

### 상품 관리하기
- [ ] 상품 정보 저장하기  
  - [ ] 상품 이름, 가격, 재고를 설정 
  - 재고는 일반 재고와 프로모션 재고를 따로 설정
- [ ] 재고 정보 업데이트 하기 
  - [ ] 재고 추가하기 
  - [ ] 재고 삭제하기 
- [ ] 가격 정보 반환하기 
...
### 재고 관리하기 
- [ ] 상품 목록 파일을 읽고 상품 정보 저장하기 
  - [ ] 새 상품이면 상품 정보를 새로 만들어 저장
  - [ ] 프로모션 상품이면 기존 상품 정보에 프로모션 재고 업데이트 하기  
- [ ] 상품을 더 빠르게 찾기 위한 인덱스 사전 만들기 
  - [ ] 상품 이름으로 현재 상품의 위치(인덱스)를 알 수 있는 사전 생성 
- [ ] 상품이름으로 상품 가져오기

### 주문 내역 관리하기 
- [ ] 사용자로부터 입력받은 상품 및 구매 수량 정보 저장하기 
- [ ] 상품 구매 수량 업데이트 하기 
  - [ ] 상품 수량 추가하기 
  - [ ] 상품 수량 제거하기 
- [ ] 프로모션 수량 계산하기 
  - [ ] 프로모션 상품 명과 증정품 갯수 정보 저장하기
- [ ] 총 구매액 계산하기(할인 적용 전)
- [ ] 프로모션 할인액 계산하기 
...
- [ ] 멤버십 할인 여부 설정하기 
- [ ] 멤버십 할인 계산하기 
  - [ ] 프로모션 제외한 금액의 30%로 계산하기
  - [ ] 할인이 8000원이 넘지 않도록 하기 
- [ ] 내실돈 계산하기
  - 최종 결제 금액 : 총구매액 - 프로모션 할인액 - 멤버십 할인액 

### 고객에게 출력하기 
- [ ] 환영 인사 출력하기 
- [ ] 현재 보유 상품 내역 출력하기 
  - [ ] 상품명, 가격, 프로모션 이름, 재고 안내하기
  - [ ] 재고가 0개이면 재고없음 출력하기
- [ ] 프로모션 상품에 대해 적게 가져온 경우, 추가 여부에 대한 안내 메시지 출력하기 
- [ ] 프로모션 상품 재고가 부족하여 혜택 없이 결제할 경우, 정가 결제 여부에 대한 안내 메시지 출력하기 
- [ ] 멤버십 힐인 적용 여부 안내 메시지 출력하기
- [ ] 영수증 출력하기 
- [ ] 추가 구매 여부 안내 메시지 출력하기
- [ ] 잘못된 값 입력시, 오류 메시지와 함께 안내 메시지 출력하기
